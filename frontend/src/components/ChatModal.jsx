...
function ChatModal({ onClose }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(`${AI_API_END_POINT}/chatbot`, {
        method: 'POST',
        headers: {
      ...
      const botResponse =
        data.result || data.response || 'No response from chatbot';
  ...
  return (
    <div className="fixed bottom-16 right-4 bg-white w-[30rem] h-[30rem] shadow-lg border border-gray-300 rounded-lg flex flex-col">
      <div className="p-4 bg-purple-600 text-white text-center text-xl font-bold">
        Chatbot
      </div>
      ...
      <div className="p-4 flex border-t border-gray-300">
        ...
        <button
          onClick={sendMessage}
          className="ml-4 px-5 bg-purple-600 text-white rounded-lg"
        >
          Send
        </button>
      </div>
